...
class Truss:
    ...
    def anaylize(self, CSGcrossSections, Iteration, Population, CantileverPoint):
        ...
        # analyses
        [mainDisplacementMax, cantileverDisplacementMax, massOfTruss] = self.solve(SapModel, CantileverPoint, Iteration, Population)

        cost = self.calcCost(mainDisplacementMax, cantileverDisplacementMax, massOfTruss)
        return cost
        
        # return analysesed data

    def addToSAPFile(self, SapModel):
        for i in range(len(self.nodes)):
            Name = ""
            [a, ret] = SapModel.PointObj.AddCartesian(0, self.nodes[i][0], self.nodes[i][1], Name, str(i), 'GLOBAL', False, 0)
    ...
    def readIn(self):
        ...
        for i in range(len(Node1)):
            self.members.append([Node1[i], Node2[i]])
    ...
    def addSuports(self, SapModel):
        restraint = [[False,False,False,False,False,False],[True,True,True,False,False,False],[False,False,True,False,False,False],[False,False,True,False,False,False]]
        for i in range(len(self.supports)):
            print(self.supports[i][1])
    ...
    def save(self, SapModel, Iteration, Population):
        OutputPath = "C:\Desktop\Sap2000CodeOutput\Save_" + str(Iteration) + "_" + str(Population)
    ...
    def AddStringerLoads(self, SapModel, LoadCase, StringerElevation, BetweenDis, Magnitude, Direction):
        ...
        for i in range(len(self.members)):
            ...
            if n1[0] == StringerElevation & n1[1] == StringerElevation:
